...
def _tensor_type_to_str(tensor_type, name):
    declare_dtype = onnx.mapping.STORAGE_TENSOR_TYPE_TO_FIELD[tensor_type.elem_type].split("_")[
        0]
    declare_shape = [di.dim_param if di.dim_value < 0 else str(
        di.dim_value) for di in tensor_type.shape.dim]

    declare_shape = "" if len(declare_shape) < 1 else "[{}]".format(
        ", ".join(declare_shape))
...
def _tensor_to_str(tensor):
    ...
    declare_dtype = onnx.mapping.STORAGE_TENSOR_TYPE_TO_FIELD[tensor.data_type].split("_")[
        0]
    ...
    tensor_data_str = str(list(tensor_data.flatten()))

    if tensor_data_str[0] == "[":
        tensor_data_str = tensor_data_str[1:-1]

    tensor_bytes.write("{}{} = ".format(
        declare_dtype, declare_shape).encode('utf-8'))
...
def _node_to_str(node, bytes_io):
    ...
    bytes_io.write("    {} = {}".format(
        output_info_str, node.op_type).encode('utf-8'))
...
def onnx_model_textual(onnx_model):
    ...
    for item in onnx_model.graph.initializer:
        if len(item.raw_data) == 0:
            graph_init_dict[_format_str(item.name)] = np.reshape(np.array(item.__getattribute__(
                onnx.mapping.STORAGE_TENSOR_TYPE_TO_FIELD[item.data_type])).astype(onnx.mapping.TENSOR_TYPE_TO_NP_TYPE[item.data_type]),
                item.dims)
        else:
...
if __name__ == '__main__':
    ...
    with open("deploy.onntxt", 'w') as fp:
    ...
    # paser
    with open("deploy.onntxt", 'r') as fp:
        onnx_model_txt = fp.read()
        fp.close()
    ...
    onnx.save_model(onnx_model_new, "onnx_model_from_text.onnx")
